...
var dataset = [
	{ label: "Iris-Setosa", percentage: 10 },
	{ label: "Iris-Versicolour", percentage: 40 },
	{ label: "Iris-Virginica", percentage: 50 },
];
...
var pie = d3.pie()  // start and end angles of segments
	.value(function (d) {
		return d.percentage; // How to extract numerical data from each entry in our dataset
	})
...
tooltip.append('div')  // add divs to the tooltip define above
	.attr('class', 'percentage'); // add class 'label' on the selection
...
path.on('mouseover', function (d) {
	...
	tooltip.select('.percentage').html(d.data.percentage + '%'); // set current percentage
